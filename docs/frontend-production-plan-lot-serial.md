...
공정 PASS·소자/파장 보정·납품 등록 등 **프론트 임시 규칙**은 [`process-handling-frontend-temp.md`](./process-handling-frontend-temp.md) 참고.
...
4. 공정 PASS/FAIL
5. **`POST .../assign-product-serials`** — 제품 시리얼 **전체 문자열**(접두사+끝 4자리). UI는 접두사 읽기 전용 + 4자리 입력 (`ProductionPlanAssignProductSerialsCard.tsx`)
