...
/** `candidateYmd`가 `baseYmd` 당일 이후인지 (포함). 파싱 실패 시 `false` */
export function isYmdOnOrAfter(
  candidateYmd: string,
  baseYmd: string
): boolean {
  const candidateMs = ymdToLocalMidnightMs(candidateYmd);
  const baseMs = ymdToLocalMidnightMs(baseYmd);
  if (candidateMs == null || baseMs == null) return false;
